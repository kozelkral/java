...
public class chp11_4 {
     ...
     * Mac OS X.
     */
    private static String DATA_FILE_NAME = ".phone_book_xml";
    ...
    public static void main(String[] args) {
        ...
        if ( ! dataFile.exists() ) {
            ...
            try {
              ...
              System.out.println("The input source is not well-formed xml");
              System.exit(1);
            }
            Element root = xmldoc.getDocumentElement();
            if (!(root.getTagName().equals("phone_directory"))) {
              System.out.println("Given file is not a phone directory");
              System.exit(1);
            }
            NodeList directory = root.getChildNodes();
            for (int i = 0; i < directory.getLength(); i++) {
              Node child = directory.item(i);
              if (child instanceof Element) {
                Element dirEntry = (Element)child;
                if (!(dirEntry.getTagName().equals("entry"))) {
                  System.out.println("Bad entry found. Skpping...");
                } else {
                  name = dirEntry.getAttribute("name");
                  number = dirEntry.getAttribute("number");
        ...
        if (changed) {
            ...
            for ( Map.Entry<String,String> entry : phoneBook.entrySet() ) {
                out.println("    <entry name='" + entry.getKey() +
